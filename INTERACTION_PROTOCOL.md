...
## 8. Local Hygiene Hotfix (2026-02-16)

Для исправления ошибок воспроизводимости и артефактного дрейфа:

1. В `.gitignore` обязательно покрывать: `.venv/`, `venv/`, `**/venv/`, `ENV/`, `env/`, `*.pyc`.
2. Изменение `.gitignore` фиксируется в `DEV_LOGS.md` и `ROADMAP.md` в той же governance-сессии.
3. Ограничение: добавление в ignore предотвращает только новые артефакты; уже tracked runtime-файлы требуют отдельного решения (untrack по явному решению оператора).
